...
A while ago -- a little bit over a year -- I wrote a post with a methodical way to write clear, concise, and useful commit messages. The main argument is that this helps documentate code changes. With thoroughly explained changes, developers in the future can simply look at the commit message and better understand the context for the implementation.
